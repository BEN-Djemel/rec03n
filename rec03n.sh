...
#		̀`curl -s https://www.google.com/search?q=site:$ggdorking | grep -oP '(?<=href=")[^"]+\.'$1'\.'$2'' | grep -oP 'q=\Khttp[^"]+' >> recon/googleDork.txt`
	...
	[Nn]* ) read -p "Enter a domain attack : " mondodo
		...
		`python3 /"full_path_here"/massdns/scripts/recon.py -d $mondodo -l /"full_path_here"/massdns/lists/best-dns-wordlist.txt --o recon/massDns.txt`
		echo "chaos api initialisation..."
		export PDCP_API_KEY="your_api_key_here"
		echo "chaos process..."
		chaos -silent -d $mondodo -o recon/chaos.txt
		echo "sudomy process..."
		/"full_path_here"/Sudomy/sudomy -d $mondodo --dnsx -s Shodan,Certspotter,Binaryedge,Hackertarget,Threatminer,CrtSH,DnsDB,BufferOver,Sypse,Threatcrowd,Dnsdumpster,Riddler,Webarchive,SecurityTrails,RapidDNS,AlienVaultCommonCrawl,FBcert,URLScan,RiskIQ
		aujourd=$(date +%m-%d-%Y)
		#regrouper les resultats en un seul fichier
